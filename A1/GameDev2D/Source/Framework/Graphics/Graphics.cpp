...
namespace GameDev2D
{
    ...
    void Graphics::SetTextureStorage(unsigned int id, unsigned int width, unsigned int height, PixelFormat internalFormat, unsigned int levels)
    {
        ...
        else
        {
            ...
            glBindTexture(GL_TEXTURE_2D, id);
            glTexStorage2D(GL_TEXTURE_2D, levels, GetOpenGLInternalPixelFormat(internalFormat), width, height);
    ...
    void Graphics::SetTextureFilter(unsigned int id, TextureMinFilter minFilter, TextureMagFilter magFilter)
    {
        ...
        else
        {
            ...
            glGetIntegerv(GL_TEXTURE_BINDING_2D, &prevTexture);
            glBindTexture(GL_TEXTURE_2D, id);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GetOpenGLTextureMinFilter(minFilter));
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GetOpenGLTextureMagFilter(magFilter));
            glBindTexture(GL_TEXTURE_2D, prevTexture);
        }
    }
    ...
    void Graphics::SetTextureData(unsigned int id, const ImageData& imageData)
    {
        ...
        else
        {
            ...
            glGetIntegerv(GL_TEXTURE_BINDING_2D, &prevTexture);
            glBindTexture(GL_TEXTURE_2D, id);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, imageData.width, imageData.height, pixelLayout, pixelSize, &imageData.data[0]);
            glBindTexture(GL_TEXTURE_2D, prevTexture);
        }
    }
